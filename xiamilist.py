...
for i in range(1,111):
    myurl = 'http://www.xiami.com/space/lib-song/u/9945786/page/'+'%s'%i
...
list = '['
for i in range(0,len(songlist)):
    songunit = '{\n\tartist: "' + artistlist[i].replace("\"","") +\
               '",\n\tname: "' + songlist[i].replace("\"", "") + '"\n}'
    list += songunit
    if i < len(songlist) - 1:
        list += ',\n'
    print ('%d/%d')%((i+1), len(songlist))
list += ']'
file = open('xiamilove.json','w')
file.write(list.encode('utf-8'))
file.close()
print 'JSON complete'